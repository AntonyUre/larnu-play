...
let wrapper = document.querySelector(".container-card");
let info = document.querySelector(".xxx");

for (i = 0; i < course.length; i++) {
  console.log(course[i].title);
  wrapper.innerHTML += `<div class="card" id="${course[i].date}">
  ${course[i].video}
  <div class="card__text-container">
  <div><a href="#" target="_blank">${course[i].title}</a>
  ...
  <span>${course[i].description}</span></a
>`;
}
